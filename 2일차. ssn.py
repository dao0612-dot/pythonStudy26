...
#처리1 : 생년월일을 추출! -> 1, 2, 5, 6 1900년생, 나머지는 2000년생
...
#입력된 주민번호 검증 코드
if len(ssn) == 14 : #키보드로 입력된 문자열이 14자? #len함수 기억하기!
...
if ssn[6] == "-" :
    print("주민번호 7번째 구분자 인식완료")
else:
    print("주민번호 7번째 구분자가 입력되지 않았습니다.")
...
fullyear = "" #if 안쪽에서 변수를 만들면 버그가 생길 수 있다.
#"" <- null 처리용
if ssn[7] in ["1","2","5","6"] :
    fullyear = "19" + year
else :
...
#전라 48-66  경상 67-91  제주 92-95 표가 있다.
local = "" #출생지를 판단하는 문자열 null용

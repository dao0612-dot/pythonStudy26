...
#!중요 -> 인스턴스 : 객체를 복제해서 같은 변수나 메서드를 찍어내는 효과
#LMS 프로그램을 지금 처럼 콘솔에 출력하면 1인용인데 클래스로 구현하면 여러명이 접근해서 사용할 수도 있다. ->FLASK,fastAPI(웹서버)

#변수 선언시 글로벌 영역에 선언하면 session -> 값을 공유
#클래스가 아닌 일반 사항으로 처리하면 1명이 로그인하면 다른사람이 접속할 때 이미 로그인한 상태가 됨
...
def add(num):   #함수 만듬 -> num을 받음
    ...
    return result   #결과를 add에 넣어서 내보냄
#add() 함수 만듬 끝

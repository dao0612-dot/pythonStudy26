...
import os
from Member import Member #회원 객체 추가
#사용법 = member = Member() -> 객체가 생성됨
#        member.필드/메서드 = ??


class MemberService:
    def __init__(self,file_name="members.txt"): #클래스가 생성할 때 초기값 관리
    ...
    def load_members(self): #파일에서 메모리로 불러온다.
        ...
        self.members = [] #메모리에 남은 값을 초기화(어차피 불러올 텍스트 파일의 내용만 있어야하기 때문에)
    ...
    def save_members(self):
        with open(self.file_name,"w", encoding="utf=8") as f:
            for member in self.members:
                ...
                #       Member객체의 매서드를 사용하여 1줄씩 기록

    #id를 이용해 members에서 찾는 공통 메섣,
    def find_member(self, uid):
        for member in self.members: #members 리스트에서 1개씩 member 객체를 가져와
            if member.id == uid : #가져온 member객체.id와 전달받은 id가 같은지
                print(member.name," 님을 찾았습니다.")
                #예전에는 member[2]으로 찾았는데, 지금은 변수명으로 찾을 수 있음

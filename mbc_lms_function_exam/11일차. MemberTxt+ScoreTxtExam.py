...
scores = [] #지금은 비어있지만 좀 있다 메모장에 있는 내용을 가져와 리스트 처리함
#scores는 이름, 국,영,수, 총점, 평균, 등급 순으로 구성할 예정
...
def save_scores():
    ...
    with open(FILE_NAME_SCORE, "w", encoding="utf-8") as f:
        for score in scores:
            data = f"{score[0]}|{score[1]}|{score[2]}|{score[3]}|{score[4]}|{score[5]}|{score[6]}|{score[7]}|{score[8]}|{score[9]}\n"
            f.write(data)
...
def load_members(): #텍스트파일을 전체 불러와 리스트로 만듬!!! 왜??!?!?!!!? 중간 수정이 안됨
    ...
    #파일이 없으면 새파일 생성 (암기)
    if not os.path.exists(FILE_NAME): #지금 디렉토리에 FIIL_NAME이 없으면!!!
...
def load_scores():
    ...
    if not os.path.exists(FILE_NAME_SCORE):
        ...
        with open(FILE_NAME_SCORE, "r", encoding="utf-8") as f:
            for line in f:
                data = line.strip().split("|")
                data[1] = int(data[1]) if data[1] == data[1] else data[1]
                data[2] = int(data[2]) if data[2] == data[2] else data[2]
                data[3] = int(data[3]) if data[3] == data[3] else data[3]
                data[4] = int(data[4]) if data[4] == data[4] else data[4]
                data[5] = float(data[5]) if data[5] == data[5] else data[5]
                scores.append(data)
...
def score_add():
    ...
    select_id = input("점수를 추가할 학생의 id :")
    list = []
    result = False
    for i in range(len(members)):
        list.append(members[i][0])
        if select_id in list :
            result = True
            name = members[i][2]
            break

    if result == False :
        print("해당되는 학생의 id가 없습니다.")
        return
    x = select_id
    if any(x in row for row in scores) :
        print("이미 해당 학생의 점수가 존재합니다.")
        return
    ...
    if input(dedent(f"""
    해당 내용으로 성적 입력을 확정하시겠습니까?
    이름 : {name}  총점 : {total}  평균 : {avg} 
    국어 : {kor}점  {kor_grade}등급
    영어 : {eng}점  {eng_grade}등급
    수학 : {mat}점  {mat_grade}등급
    
    (y/n)
    """)) == "y":
        scores.append([name, kor, eng, mat, total, avg, kor_grade, eng_grade, mat_grade, select_id])
        save_scores()
...
def score_modify():
    ...
    score_manager_list()
    user_select = int(input("성적을 수정할 유저의 번호 :"))
    stu_idx = user_select-1
    list = []
    for score in scores:
        list.append(scores.index(score))

    if user_select in list:
        pass
    ...
    select = input(">>>")
    if select == "1":
        kor = int(input("수정할 국어 점수 :"))

        if input(f"{scores[stu_idx][0]}님의 국어 점수를 {kor}점으로 변경하시겠습니까? (y/n) :") == "y" :
            print("국어 점수 변경을 완료했습니다.")
            ...
            scores[stu_idx][1] = kor
            total = kor + scores[stu_idx][2] + scores[stu_idx][3]
            avg = total/3
            scores[stu_idx][4] = total
            scores[stu_idx][5] = avg
            scores[stu_idx][6] = kor_grade

            save_scores()

        else:
            print("국어 점수 변경을 취소합니다.")
            return

    elif select == "2":
        eng = int(input("수정할 영어 점수 :"))

        if input(f"{scores[stu_idx][0]}님의 영어 점수를 {eng}점으로 변경하시겠습니까? (y/n) :") == "y":
            print("영어 점수 변경을 완료했습니다.")
            ...
            scores[stu_idx][2] = eng
            total = scores[stu_idx][1] + eng + scores[stu_idx][3]
            avg = total / 3
            scores[stu_idx][4] = total
            scores[stu_idx][5] = avg
            scores[stu_idx][7] = eng_grade

            save_scores()

        else:
            print("영어 점수 변경을 취소했습니다.")
            return

    elif select == "3":
        mat = int(input("수정할 수학 점수 :"))

        if input(f"{scores[stu_idx][0]}님의 수학 점수를 {mat}점으로 변경하시겠습니까? (y/n) :") == "y":
            print("수학 점수 변경을 완료했습니다.")
            ...
            scores[stu_idx][3] = mat
            total = scores[stu_idx][1] + scores[stu_idx][2] + mat
            avg = total / 3
            scores[stu_idx][4] = total
            scores[stu_idx][5] = avg
            scores[stu_idx][8] = mat_grade
        ...
        if input(dedent(f"""
            {scores[stu_idx][0]}님의 점수를 다음과 같이 변경하시겠습니까?
            국어 : {kor}
            영어 : {eng}
            수학 : {mat}            
            (y/n)
...
def score_delete():
    ...
    score_manager_list()
    user_select = int(input("성적을 삭제할 유저의 번호 :"))
    stu_idx = user_select - 1
    ...
    if stu_idx in list:
        ...
        print("존재하지 않는 학생 번호입니다.")
        return

    if input(f"정말로 {scores[stu_idx]}님의 성적을 삭제하시겠습니까? (y/n) :") == "y":
        print(f"{scores[stu_idx]}님의 성적을 삭제했습니다.")
        scores.pop(stu_idx)
...
def member_list():
    for idx, member in enumerate(members):
        print(f"{idx+1}. {member[2]} | {member[0]}")


def member_admin_list():
    for idx, member in enumerate(members):
        print("-" * 15)
        print(f"{idx+1}. {member[2]} | {member[0]} | {member[3]} | {member[4]}")
        print("-" * 15)
...
def score_main_menu():
    ...
    while True:
        ...
        elif scores_select == "2":
            if members[session][3] == "manager":
                if scores == []:
                    print("점수가 입력된 학생이 없습니다.")
                    continue
                score_manager_list()
...
def score_manager_list():
    print(f"{'번호':5}{'이름':5}{'국어':4}{'영어':4}{'수학':4}{'총점':5}{'평균':5}")
    print("-" * 30)
    for score in scores :
        print(f"{scores.index(score)+1:<5}{score[0]:5}{score[1]:^5}{score[2]:>3}{score[3]:>5}{score[4]:>6}{score[5]:>7}")
...
def score_user_list():
    ...
    user = None
    for score in scores :
        if members[session][0] == score[9] :
            user = scores.index(score)
    ...
    국어 : {scores[user][1]:3}점 | {scores[user][6]:>1}등급
    영어 : {scores[user][2]:3}점 | {scores[user][7]:>1}등급
    수학 : {scores[user][3]:3}점 | {scores[user][8]:>1}등급
    
    총점 : {scores[user][4]:3}점 | 평균 : {scores[user][5]:4}점
    -----------------------------------------------------

    """))


def score_modify_menu():
    print(dedent(f"""
    ------- 성적 수정 항목 -------
...
load_scores() #프로그램 시작 시 파일을 불러오기!!! -> 1번만! 2차원 배열로 불러옴!
while run:  # 메인 프로그램 실행 코드
    ...
    if select == "1":  # 회원가입 코드
        ...
        if session is None:
            print("로그인이 필요한 서비스입니다.")
            continue
        score_main_menu() #성적관리 시스템 함수 호출
